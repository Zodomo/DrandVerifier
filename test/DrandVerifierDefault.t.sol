...
contract DrandVerifierDefaultTest is Test {
    ...
    function testVerifyAcceptsLatestLiveDefaultRoundViaFFI() public {
        (uint64 round, bytes memory previousSignature, bytes memory signatureUncompressed) = _fetchLatestDefaultRoundFromApi();
    ...
    function _fetchLatestDefaultRoundFromApi()
        internal
        returns (uint64 round, bytes memory previousSignature, bytes memory signatureUncompressed)
    {
        ...
        signatureUncompressed = _decompressG2SignatureViaFFI(signatureCompressedHex);
    }

    function _decompressG2SignatureViaFFI(string memory compressedHex) internal returns (bytes memory) {
        string[] memory command = new string[](4);
        command[0] = "python3";
        command[1] = "-c";
        command[2] =
            "import sys;from py_ecc.bls.g2_primitives import signature_to_G2;from py_ecc.optimized_bls12_381 import normalize;h=sys.argv[1];s=bytes.fromhex(h);p=signature_to_G2(s);x,y=normalize(p);to_i=lambda v:int(v.n) if hasattr(v,'n') else int(v);b=lambda z:int(z).to_bytes(48,'big');x0=to_i(x.coeffs[0]);x1=to_i(x.coeffs[1]);y0=to_i(y.coeffs[0]);y1=to_i(y.coeffs[1]);print((b(x1)+b(x0)+b(y1)+b(y0)).hex(),end='')";
        command[3] = compressedHex;
